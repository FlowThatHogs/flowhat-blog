...
딱 5개만 써본다면 여기서 시작하면 된다.

### `/office-hours`

무엇을 만들어야 할지 아직 정확하지 않을 때 쓴다. 기능 요청만 말하지 말고 사용자 문제를 설명하는 게 좋다. 목표는 더 선명한 문제 정의와 더 작은 첫 버전을 얻는 것이다.

좋은 프롬프트:

```text
/office-hours
회의가 너무 많은 사람들을 위한 daily briefing 기능을 만들고 싶다.
진짜 사용자 문제가 무엇인지, 가장 작지만 유용한 버전이 무엇인지 정리해줘.
```

### `/plan-ceo-review`

아이디어는 잡혔지만 범위가 너무 클 수 있을 때 쓴다. 범위를 줄이고, 약한 가정을 공격하고, v1에 들어갈 것과 빠질 것을 나누게 한다.

좋은 프롬프트:

```text
/plan-ceo-review
이 기능 계획을 리뷰해줘. 범위에는 엄격하게 봐줘. 구현 전에 무엇을 빼야 할까?
```

### `/plan-eng-review`

구현 전에 여러 파일, 데이터 흐름, API, 인증, 마이그레이션, 상태 관리가 걸려 있을 때 쓴다. 여기서 원하는 건 더 큰 야망이 아니다. 리스크, edge case, 테스트 계획이다.

좋은 프롬프트:

```text
/plan-eng-review
이 계획의 아키텍처 리스크, 숨은 가정, edge case, 코딩 전에 필요한 테스트를 점검해줘.
```

### `/review`

Claude Code가 코드를 쓴 뒤에 쓴다. 구현의 연장이 아니라 별도 리뷰어처럼 다루는 게 좋다. 프로덕션 버그, 빠진 케이스, 계획과 코드의 불일치를 보게 한다.

좋은 프롬프트:

```text
/review
현재 브랜치를 받아들이기 전에 리뷰해줘. 대충 보면 지나칠 수 있지만 실제로는 터질 버그에 집중해줘.
```

### `/qa`

실제 브라우저 플로우가 있을 때 쓴다. staging URL과 테스트할 경로를 준다. 일반 코드 리뷰로 놓치기 쉬운 제품 레벨 실패를 잡는 단계다.

좋은 프롬프트:

```text
/qa https://staging.example.com
회원가입, 온보딩, 첫 번째 성공 액션까지 테스트해줘. 버그를 보고하고 수정 여부도 다시 확인해줘.
```

이 다음에 `/ship`을 붙일 수는 있다. 하지만 `/ship`을 시작점으로 보면 안 된다. ship은 마지막 단계지, 워크플로 자체가 아니다.
