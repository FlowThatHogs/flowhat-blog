...
Anthropic이 Claude Code에 Auto Mode를 넣은 이유는, 코딩 에이전트가 정말 쓸 만해지기 시작하면 수동 승인 방식이 더 이상 잘 버티지 못하기 때문이다.

짧게 말하면 그렇다.

조금 더 길게 말하면, 코딩 에이전트 제품의 경쟁 축이 달라지고 있다. 이제는 단순히 모델 성능만 보는 게 아니라, 에이전트가 어디까지 혼자 일할 수 있는지, 어디서부터 승인이 필요한지, 그리고 제품이 얼마나 많은 마찰을 줄이면서도 무모해지지 않는지가 중요해지고 있다.

그래서 이 기능이 의미가 있다.

> English version: [/blog/2026-03-31-why-anthropic-added-auto-mode-to-claude-code/](/blog/2026-03-31-why-anthropic-added-auto-mode-to-claude-code/)
...
문제는 코딩 에이전트가 아직 약하다는 것이 아니다.

문제는 에이전트가 실제 멀티스텝 작업에 쓸 만해지기 시작하면, 끊임없는 승인 요청이 인간을 병목으로 만들어버린다는 점이다.

강한 샌드박스는 도움이 되지만 실제 워크플로를 제한한다. 반대로 완전히 열린 모드는 마찰은 줄여주지만 신뢰 경계를 너무 넓힌다.

Auto Mode는 Anthropic이 그 중간 지점을 차지하려는 시도다.

이건 Anthropic의 최근 엔지니어링 방향과도 맞닿아 있다. 이 회사는 계속 하네스 설계, 장시간 작업 구조, eval, subagent, 모델 바깥의 시스템 설계를 강조해왔다. Auto Mode 역시 그 연장선에 있다.
...
Auto Mode가 Claude Code를 완전히 다른 등급의 코딩 에이전트로 바꿔주는 것은 아니다. 대신 실제 병목 하나를 건드린다. 에이전트가 멀티스텝 작업을 하기 시작할수록 인간 감독이 너무 많이 들어가는 문제다.
...
Anthropic은 Auto Mode로 바로 그 문제를 풀려는 중이다. 그리고 그래서 이 발표는 Claude Code 하나의 기능 업데이트 이상으로 의미가 있다.
