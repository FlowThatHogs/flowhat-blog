...
Anthropic에 따르면 Claude Code 사용자는 permission prompt의 대부분을 결국 승인한다. 공식 글에서는 사용자가 93%의 승인 요청을 받아들인다고 설명한다. 이 말은 수동 승인 레이어는 남아 있어도, 실제로는 그 가치가 이미 약해졌다는 뜻이다.
...
OpenAI의 Codex는 **sandbox mode**와 **approval policy**를 명시적으로 분리한다. 기본적으로는 네트워크 접근을 제한하고 샌드박스를 사용하며, 그 경계를 넘는 행동에 대해서만 승인을 요구한다.

Cline은 같은 문제를 더 세분화된 permission 모델로 다룬다. 어떤 종류의 행동은 자동 승인하고, 어떤 행동은 계속 사용자 검토를 거치게 할지를 사용자가 나눌 수 있다.
